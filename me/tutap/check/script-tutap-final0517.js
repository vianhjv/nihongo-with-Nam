...
// ==========================================
// THUẬT TOÁN "TẦM NHÌN HẸP" (MICRO-WINDOW) - NGĂN CHẶN NHẢY CÓC ĐIỆP NGỮ
// ==========================================
let lastMatchedText = ""; 
...
function processFuzzyMatching(spokenText) {
    ...
    let recentSpokenWords = spokenWordsRaw.slice(-8).map(w => normalizeText(w)); 
    if (recentSpokenWords.length < 2) return;

    // LUẬT KIỂM SOÁT TỪNG PHẦN: CHỈ CHO PHÉP AI NHÌN XA TỐI ĐA 5 TỪ.
    // Điều này có nghĩa là màn hình sẽ KHÔNG BAO GIỜ nhảy vượt qua 1 dòng kinh.
    let MAX_LOOKAHEAD = 5; 
    let scriptWords = [];
    
    // Giới hạn an toàn, tránh lỗi cuối bài
    let limit = Math.min(currentWordIndex + MAX_LOOKAHEAD, masterWords.length);
    
    for (let i = currentWordIndex; i < limit; i++) {
        scriptWords.push({ index: i, text: normalizeText(masterWords[i].text) });
    }

    let matchedScriptIndex = -1;

    // Bắt buộc phải tìm thấy 2 từ liên tiếp khớp nhau TRONG PHẠM VI 5 TỪ NÀY
    ...
    // Nếu tìm thấy trong phạm vi hẹp này, mới cho phép di chuyển
